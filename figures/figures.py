...
def plot_weekly_events_and_gps(gps_speed,gps_time_vect,noise_vect,noise_date_vect,detection_times,daily_stacks,stacks,colors,baz_bounds,backazimuths):
    ...
    ax_twin.set_ylabel("\n  Seismic Noise RMS \n($10^{-6}$ m/s)",fontsize=15,color="#0075f2")
    ax_twin.bar(noise_date_vect,noise_vect,width=7,facecolor="#08bdff",edgecolor='k')
    ...
    ax_twin.text(datetime(2012,5,15),3e-6,"Secondary rift \nbranch opens")
    ax_twin.text(datetime(2013,9,1),3e-6,"Iceberg B-31 \ncalves")
    ax_twin.text(datetime(2013,4,20),8e-6,"Preliminary \ncalving near \nrift tip")
    ...
    for i in range(len(baz_bounds)):
        ...
        ax[ax_ind[i],0].set_ylabel("Events",fontsize=15,color=colors[i])
        ax[ax_ind[i],0].set_ylim(0,120)
        ax[ax_ind[i],0].set_title(letters[i],loc="left",fontsize=15)
        ax[ax_ind[i],0].vlines([datetime(2012,5,9),datetime(2013,7,1),datetime(2013,11,7)],0,120,colors=['dimgray','dimgray'],linestyles='dashed')
        ...
        norm_daily_stacks[np.isnan(norm_daily_stacks)] = 0
