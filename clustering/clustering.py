...
def plot_clusters(c,cluster,centroid,waveforms,correlation_coefficients,shifts):
    ...
    median_amp = np.median(np.amax(np.abs(aligned_cluster_events[sort_index,:]),axis=1,keepdims=True))
    norm_centroid = centroid/np.max(np.abs(centroid))*median_amp*1000
    for w in range(len(aligned_cluster_events)):
        ax[0].plot(t,aligned_cluster_events[w]*1000,'k',alpha=0.005)
    ax[0].plot(t,norm_centroid,linewidth=1)
    ax[0].set_ylim([-1.5*np.nanmax(abs(norm_centroid)),1.5*np.nanmax(abs(norm_centroid))])
